...
export class Repository {
  ...
  * Add entity
  */
  createToDoItemAsync(todoitem: ToDoItemInfo) {
  ...
  * Replace Entity
  */
  replaceToDoItemAsync(todoItem: ToDoItemInfo) {
    this.http
  ...
  * Update entity
  */
  updateToDoItemAsync(id: number, changes: Map<string, any>) {
  ...
  deleteToDoItemAsync(id: number) {
    this.http.delete<boolean>(`${itemsUrl}/${id}`).subscribe({
      next: (result) => {
        if (result === true) {
